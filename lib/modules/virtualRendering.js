import { combineLines, getChangedRanges, renderLine, setLineNodesRanges } from '../rendering/rendering';
import { h, patch } from '../rendering/vdom';
import { setSelection } from '../rendering/selection';
import { isEqual } from '@typewriter/delta';
export function virtualRendering(editor) {
    let start = 0;
    let end = 0;
    let heightMap = [];
    let children = [];
    ...
    let lineSelection = null; // not doc index but Combined index
    let toRender;
    ...
    const offResize = onResize(viewport, (width, height, changed) => {
        ...
        if (changed & WIDTH)
            heightMap = []; // content may be different heights, recalculate everything
        update();
    });
    ...
    function update() {
        ...
        if (count >= 20)
            console.error('Updated virtual max times');
    ...
    function renderToDom() {
        ...
        for (let i = 0, space = 0; i < items.length; i++) {
            if (renderSet.has(i)) {
                ...
                if (i === 1)
                    spacerKey = 'spacer-start';
                else if (i === items.length - 2)
                    spacerKey = 'spacer-end';
                ...
                if (!space)
                    spacerMarginTop = getMarginBetween(i, -1);
    ...
    function updateHeights() {
        children = Array.from(editor.root.children).filter(child => child.className !== '-spacer-');
        ...
        averageHeight = Math.round(getMarginBetween(0, -1, heights) +
    ...
    function onChange(event) {
        const { old, doc } = editor.modules.decorations || event;
...
function getScrollParent(node) {
    while (node && node !== node.ownerDocument.scrollingElement) {
        if (scrollable.test(getComputedStyle(node).overflowY))
            return node;
...
const HEIGHT = 2;
const BOTH = 3;
function onResize(node, callback) {
...
function selectedLineIndexes([from, to], lines) {
    let first = 0, last = 0;
...
//# sourceMappingURL=virtualRendering.js.map